...
function main() {
    ...
    hyperion = new Hyperion( adapter.config['address']  || '127.0.0.1', 19446 );
	var Type = require('type-of-is');
	hyperion.on('connect', function(){
		...
		hyperion.getServerinfo(function( err, result ){
			...
			if (my_effects){  //some effects present so create device effects
				...
				var my_active_effect_obj = {type: 'state', common: {role: 'text.url', type: 'text', name:'activeEffects'}, native:{id: 'activeEffects'}};
                ...
                for (var effect in my_effects){
					...
					var my_color_switch = {type: 'state', common: {role: 'evel.color.rgb',  name: 'activeColor'}, native:{id: 'effects' + my_effect_friendly_name + '.activator'}};
                    adapter.setObject('effects' + '.' + my_effect_friendly_name  + '.activator' , my_activator_switch);
